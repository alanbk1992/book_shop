...
document.querySelectorAll("[data-view]").forEach(t => {
	t.addEventListener("click", e => {
		t.targetEl || (e = e.target.closest(e.target.getAttribute("data-view-target")) || document.querySelector(e.target.getAttribute("data-view-target")), t.targetEl = e), "fullscreen" == t.getAttribute("data-view") ? t.classList.contains("active") ? t.targetEl.requestFullscreen ? t.targetEl.requestFullscreen() : t.targetEl.webkitRequestFullscreen && t.targetEl.webkitRequestFullscreen() : document.exitFullscreen ? document.exitFullscreen() : document.webkitExitFullscreen && document.webkitExitFullscreen() : (t.targetEl.classList.toggle("content-full-page"), body.classList.toggle("body-sc"))
	})
}), document.querySelectorAll(".btn-close-card").forEach(t => t.addEventListener("click", e => t.closest(".card").remove())), document.querySelectorAll(".nav-toggler").forEach(e => {
	e.addEventListener("click", () => {
		window.innerWidth < 992 || !root.classList.contains("mn--min") && !root.classList.contains("mn--max") ? root.classList.toggle("mn--show") : (root.classList.toggle("mn--min"), root.classList.toggle("mn--max"))
	})
}), document.querySelectorAll(".sidebar-toggler").forEach(e => {
	e.addEventListener("click", () => root.classList.toggle("sb--show"))
}), document.addEventListener("click", e => {
	e.target.classList.contains("root") && (root.classList.remove("mn--show"), root.classList.contains("sb-stuck") || root.classList.remove("sb--show"))
});
...
mainNav && (isMiniNav = root.classList.contains("mn--min"), miniNavTogglers = [...mainNav.querySelectorAll(".mininav-toggle")], miniNavContents = [...mainNav.querySelectorAll(".mininav-content")], mainNav.addEventListener("transitionend", e => {
	e.target.classList.contains("mainnav") && "max-width" == e.propertyName && buildNav()
}), miniNavContentsCollapse = miniNavContents.map(e => {
	if (e.parentElement.querySelector(".mininav-toggle.active") && (!isMiniNav || window.innerWidth < 992)) {
		const n = e.parentElement;
		n.classList.add("open");
		var t = new bootstrap.Collapse(e, {
			toggle: !0
		});
		return e.addEventListener("transitionend", () => n.classList.remove("open"), {
			once: !0
		}), t
	}
...
const popperOptions = {
	...
	updateNav = () => {
		miniNavTogglers = [...mainNav.querySelectorAll(".mininav-toggle")], miniNavContents = [...mainNav.querySelectorAll(".mininav-content")], buildNav()
	},
	addVariables = e => {
		...
		e._mainnav = {
			...
			collapse: function() {
				for (var e of miniNavContentsCollapse)
					if (e._element === t) return e
			}()
		}, (e._mainnav.target.toggler = e)._mainnav.target.addEventListener("show.bs.collapse", bsCollapseShow), e._mainnav.target.addEventListener("shown.bs.collapse", bsCollapseShow)
	},
	bsCollapseHide = e => {
		e.target.classList.contains("mininav-content") && (!isMiniNav || window.innerWidth < 992 ? e.target.toggler.classList.add("collapsed") : e.target.removeEventListener("hide.bs.collapse", bsCollapseHide))
	},
	bsCollapseShow = e => {
		if (e.target.classList.contains("mininav-content"))
			if (!isMiniNav || window.innerWidth < 992) e.target.toggler.classList.remove("collapsed");
			else try {
				e.target.popper.update()
			} catch (e) {}
	},
	hideAllMiniNavContent = e => {
		992 <= window.innerWidth && (!mainNav.contains(e.target) || e.target.classList.contains("mainnav__top-content")) && miniNavContentsCollapse.map(e => e.hide())
	},
	toggleContent = e => {
		if (!e.target._mainnav.target.classList.contains("nav-label") || isMiniNav && !(window.innerWidth < 992)) {
			const t = e.target._mainnav;
			t.islink && e.preventDefault(), miniNavContentsCollapse.map(async e => {
				e._element.contains(t.target) || e.hide()
			}), "mouseenter" == e.type ? t.collapse.show() : t.collapse.toggle(), ["click", "touchend"].forEach(e => document.addEventListener(e, hideAllMiniNavContent, {
				once: !0
			}))
		}
	},
	buildNav = () => {
		...
		let e = null;
		isMiniNav || (e = mainNav.querySelector(".has-sub > .mininav-toggle.nav-link.active + .mininav-content.nav")), e && e.classList.add("show"), miniNavTogglers.map(t => {
			t._mainnav || addVariables(t), t.classList.add("collapsed"), !isMiniNav || window.innerWidth < 992 ? (t.addEventListener("click", toggleContent), ["mouseenter", "touchend"].forEach(e => t.removeEventListener(e, toggleContent)), null != t._mainnav.target.popper && (t._mainnav.target.popper.setOptions({
				scroll: !1,
				resize: !1
			}), t._mainnav.target.popper.destroy()), t._mainnav.target.addEventListener("hide.bs.collapse", bsCollapseHide)) : (miniNavContentsCollapse.map(async e => e.hide()), t._mainnav.target.popper = Popper.createPopper(t, t._mainnav.target, popperOptions), t.removeEventListener("click", toggleContent), ["mouseenter", "touchend"].forEach(e => t.addEventListener(e, toggleContent)), t._mainnav.target.removeEventListener("hide.bs.collapse", bsCollapseHide))
		})
	};
	...
	a.map(o => {
		o.addEventListener("click", t => {
			if (t.preventDefault(), !o.classList.contains(".active")) {
				...
				t = o.querySelector("img").getAttribute("src").replace("thumbs", "bg");
...
if (document.getElementById("_dm-settingsContainer")) {
	...
	f.boxedLayoutRadio.addEventListener("changed", e => {
		...
		const t = e.target;
		t.querySelector("option[selected]").removeAttribute("selected"), t[t.selectedIndex].setAttribute("selected", !0), body.classList.remove("in-quart", "out-quart", "in-back", "out-back", "in-out-back", "steps", "jumping", "rubber"), body.classList.add(t.value)
	...
	[...document.querySelectorAll("._dm-offcanvasBtn")].map(e => {
		e.addEventListener("click", () => {
			i.className = `offcanvas ${e.value}`, i.style = "transition-duration: 0s", k.dispatchEvent(new Event("click")), l.addEventListener("transitionend", () => {
				i.style = "", j.show()
			}, {
				once: !0
			})
	...
	const o = document.getElementById("dm_colorSchemesContainer"),
		...
		r = t => {
			let o = null;
			return [...document.getElementsByTagName("link")].map(e => e.href.includes(t) ? o = e : null), "bootstrap" == t ? bootstrapLinkEl = o : niftyLinkEl = o, o
		},
		s = r("bootstrap.").getAttribute("href"),
		t = r("nifty.").getAttribute("href"),
	...
	const y = e => {
			...
			return t.setAttribute("rel", "stylesheet"), t.setAttribute("href", e), t.addEventListener("load", A, {
				once: !0
			}), t
		},
		z = (e, o) => {
		...
		A = () => {
			if (b++, !(b < v)) {
				bootstrap.Offcanvas.getInstance("#_dm-settingsContainer").hide(), body.classList.remove("_dm-load-scheme-css");
	...
	const D = {
		...
		E = () => new Promise((t, o) => {
			if (document.getElementById("_dm-jsOverlayScrollbars")) t("done");
				...
				e.setAttribute("id", "_dm-cssOverlayScrollbars"), e.setAttribute("rel", "stylesheet"), e.setAttribute("href", `${u}/vendors/overlayscrollbars/overlayscrollbars.min.css`), document.querySelector("head").append(e), e = document.createElement("script"), e.setAttribute("id", "_dm-jsOverlayScrollbars"), e.setAttribute("src", `${u}/vendors/overlayscrollbars/OverlayScrollbars.min.js`), document.body.append(e), e.addEventListener("load", t, {
					once: !0
				}), e.onerror = o
			}
		});
	document.getElementById("_dm-bodyScrollbarCheckbox").addEventListener("change", async e => {
		await E(), e.target.checked ? d = OverlayScrollbars(body, D) : d.destroy()
	}), document.getElementById("_dm-sidebarsScrollbarCheckbox").addEventListener("change", async e => {
		await E(), e.target.checked ? c = OverlayScrollbars(document.querySelectorAll(".scrollable-content"), D) : c.map(e => e.destroy())
	})
}
